...
class NodeAttention(torch.nn.Module):
    def __init__(self, d_model, n_attn_heads, d_attn=None, adaln=False):
        ...
        self.feat_norm = torch.nn.LayerNorm(d_model) if adaln is False else None
    ...
    def forward(self, node_feats, messages, adj_matrix, gate=None, shift=None, scale=None):
        ...
        node_feats = self.feat_norm(node_feats) if self.feat_norm is not None else node_feats * (1 + scale) + shift
