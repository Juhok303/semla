...
class MolecularCFM(L.LightningModule):
    ...
    def training_step(self, batch, b_idx):
        ...
        losses = self._loss(data, interpolated, predicted)#, codes=None)
    ...
    def _loss(self, data, interpolated, predicted):#, codes=None):
        ...
        #contrastive_loss = self._constrastive_loss(codes)
        ...
        contrastive_loss = 0#constrastive_loss.mean() * self.contrastive_loss_weight
    ...
    def _distill_loss(self, data, predicted, eps=1e-3):
        ...
        charge_loss = self._charge_loss(data, predicted)
        #contrastive_loss = self._constrastive_loss()
    ...
    def _contrastive_loss(self):#, codes=None):#, data, predicted, eps=1e-3):
        #pred_logits = predicted["charges"]
        #charges = data["charges"]
        #mask = data["mask"]
        #batch_size, num_atoms, _ = pred_logits.size()
        ...
        #n_atoms = mask.sum(dim=1) + eps
        contrastive_loss = 0#(charge_loss * mask).sum(dim=1) / n_atoms
