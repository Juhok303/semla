...
class EdgeMessages(torch.nn.Module):
    def __init__(self, d_model, n_coord_sets, d_message, d_latent=None, d_edge=None, d_ff=None, d_out=None, eps=1e-6):    #changed
        super().__init__()

        d_latent = d_model if d_latent is None else d_latent    #changed        
        edge_feats = 0 if d_edge is None else d_edge
        
        d_ff = d_message if d_ff is None else d_ff     #changed
        
        d_out = d_message if d_out is None else d_out    #changed
        
        in_feats = (d_latent * 2) + n_coord_sets + d_message + edge_feats   #changed
        ...
        self.message_norm = torch.nn.LayerNorm(d_message)    #TO DO: Discuss LayerNorm or CoordNorm
        self.edge_norm = torch.nn.LayerNorm(d_edge) if d_edge is not None else None

        self.node_proj = torch.nn.Linear(d_model, d_latent)    #changed
        self.message_mlp = torch.nn.Sequential(
            torch.nn.Linear(in_feats, d_ff), torch.nn.SiLU(inplace=False), torch.nn.Linear(d_ff, d_out)
        )

        def forward(self, coords, node_feats, node_mask, messages, edge_feats=None):    #changed
        ...
        Args:
            ...
            node_mask (torch.Tensor): Mask for nodes, shape [batch_size, n_sets, n_nodes], 1 for real, 0 otherwise
            messages (torch.Tensor): Message tensor, shape [batch_size, n_nodes, n_nodes, d_message]    #changed
        ...
        messages = self.message_norm(messages)    #changed
        ...
        in_feats = torch.cat((node_pairs, coord_feats, messages), dim=3)    #changed
        
        if edge_feats is not None:
            edge_feats = self.edge_norm(edge_feats)
            in_feats = torch.cat((in_feats, edge_feats), dim=-1)    #changed

        return self.message_mlp(in_feats)    #changed
...
class NodeAttention(torch.nn.Module):
    def __init__(self, d_model, d_message, d_edge, n_attn_heads, d_attn=None):    #changed
        super().__init__()
        ...
        self.feat_norm = torch.nn.LayerNorm(d_model)    #TO DO: AdaLN
        
        self.in_proj = torch.nn.Linear(d_model, d_attn)
        self.edge_proj = torch.nn.Linear(d_edge, d_message)    #changed
        self.out_proj = torch.nn.Linear(d_attn, d_model)


    def forward(self, node_feats, messages, edge_feats, adj_matrix):    #changed
        ...
        Args:
            ...
            edge_feats (torch.Tensor): Edge feature tensro, shape [batch_size, n_nodes, d_edge]    #changed
        ...
        attn_mask = adj_to_attn_mask(adj_matrix)    #TO DO: Discuss how to mask and to weight
        messages = messages + attn_mask.unsqueeze(3)    #TO DO
        attentions = torch.softmax(messages, dim=2)    #TO DO

        edge_proj = self.edge_proj(edge_feats)    #changed
        context = messages * edge_proj    #changed
        
        node_feats = self.feat_norm(node_feats)    #TO DO: AdaLN(context)
        ...
        attentions = attentions.movedim(-1, 1).flatten(0, 1)    #TO DO: Discuss how to weight
        ...
        weights = torch.sqrt((attentions**2).sum(dim=-1))    #TO DO: Discuss how to deal with GNN-VPA
...
class LengthsMLP(torch.nn.Module):
    def __init__(self, d_model, n_coord_sets, d_edge, d_ff=None):    #changed
        super().__init__()

        d_ff = d_model * 4 if d_ff is None else d_ff

        self.node_ff = torch.nn.Sequential(
            torch.nn.Linear(d_model + n_coord_sets + d_edge, d_ff), torch.nn.SiLU(inplace=False), torch.nn.Linear(d_ff, d_model)    #changed
        )

    def forward(self, coord_sets, node_feats, edge_feats):    #changed
        ...
        Args:
            coord_sets (torch.Tensor): Coordinate tensor, shape [batch_size, n_sets, n_nodes, 3]
            node_feats (torch.Tensor): Node feature tensor, shape [batch_size, n_nodes, d_model]
            edge_feats (torch.Tensor): Edge feature tensor, shape [batch_size, n_nodes, d_edge]    #changed
        ...
        in_feats = torch.cat((node_feats, lengths, edge_feats), dim=2)    #changed
...
class EquivariantMLP(torch.nn.Module):
    def __init__(self, d_model, n_coord_sets, d_edge, proj_sets=None):    #changed
        super().__init__()
        ...
        self.coord_proj = torch.nn.Linear(n_coord_sets, proj_sets, bias=False)
        self.edge_proj = torch.nn.Sequential(    #changed
            torch.nn.Linear(d_edge, proj_sets), torch.nn.SiLU(inplace=False), torch.nn.Linear(proj_sets, proj_sets)    #changed
        )    #changed
        self.attn_proj = torch.nn.Linear(proj_sets, n_coord_sets, bias=False)
    ...
    def forward(self, coord_sets, node_feats, edge_feats):    #changed
        ...
        # edge_feats shape [B, 1, N, P]    #changed
        # int_feats shape [B, 1, N, P]    #changed
        inv_feats = self.node_proj(node_feats).unsqueeze(1)
        proj_sets = self.coord_proj(coord_sets.transpose(1, -1))
        edge_feats = self.edge_proj(edge_feats).unsqueeze(1)    #changed

        int_feats = inv_feats * edge_feats    #changed

        # Outer product with interaction features is equivariant, then sum over original coord sets    #changed
        attentions = int_feats.unsqueeze(-1) * proj_sets.unsqueeze(-2)    #changed
...
class NodeFeedForward(torch.nn.Module):
    def __init__(self, d_model, n_coord_sets, d_edge, d_ff=None, proj_sets=None, d_hidden=None, coord_norm="length"):    #changed
        super().__init__()

        self.node_norm = torch.nn.LayerNorm(d_model)
        self.coord_norm = CoordNorm(n_coord_sets, norm=coord_norm)
        self.edge_norm_in = torch.nn.LayerNorm(d_edge)    #changed
        self.edge_norm_node_out = torch.nn.LayerNorm(d_edge)    #changed
        self.edge_norm_coord_out = torch.nn.LayerNorm(d_edge)    #changed

        d_hidden = d_model+proj_sets if d_hidden is None else d_hidden
        self.edge_mlp = torch.nn.Sequential(
            torch.nn.Linear(d_edge, d_hidden), torch.nn.SiLU(inplace=False), torch.nn.Linear(d_hidden, 2 * d_edge)
        )
        
        self.invariant_mlp = LengthsMLP(d_model, n_coord_sets, d_edge, d_ff=d_ff)    #changed
        self.equivariant_mlp = EquivariantMLP(d_model, n_coord_sets, d_edge, proj_sets=proj_sets)    #changed

    def forward(self, coord_sets, node_feats, node_mask, edge_feats):    #changed
        """Pass data through the layer
        ...
        Args:
            ...
            node_feats (torch.Tensor): Node feature tensor, shape [batch_size, n_nodes, d_model]
            node_mask (torch.Tensor): Mask for nodes, shape [batch_size, n_sets, n_nodes], 1 for real, 0 otherwise
            edge_feats (torch.Tensor): Edge feature tensor, shape [batch_size, n_nodes, n_nodes, d_edge]
        ...
        edge_feats = self.edge_norm_in(edge_feats)    #changed

        #node_edge, coord_edge: shape [batch_size, n_nodes, d_edge]
        node_edge, coord_edge = self.edge_mlp(edge_feats).sum(2).chunk(2, dim=-1)    #changed
        node_edge = self.edge_norm_node_out(node_edge)    #changed
        coord_edge = self.edge_norm_coord_out(coord_edge)    #changed
        
        out_node_feats = self.invariant_mlp(coord_sets, node_feats, node_edge)    #changed
        out_coord_sets = self.equivariant_mlp(coord_sets, node_feats, coord_edge)    #changed
...
class EquiMessagePassingLayer(torch.nn.Module):
    def __init__(
        ...
        self.d_edge_out = d_edge_out if d_edge_out is not None else d_edge_in
        ...
        d_message_out = d_message_out + d_edge_out    #changed
        ...
        self.message_ff = EdgeMessages(
            d_model, d_message, d_message_out, n_coord_sets, d_edge_in, d_ff=d_message_hidden, eps=eps    #changed
        )
        self.coord_attn = CoordAttention(n_coord_sets, self.d_coord_message, coord_norm=coord_norm, eps=eps)    #TO DO
        self.node_attn = NodeAttention(d_model, n_attn_heads, d_attn=d_attn)    #TO DO
    ...
    def forward(self, coords, node_feats, adj_matrix, node_mask, edge_feats, messages):    #changed
        """Pass data through the layer
        ...
        Args:
            ...
            messages (torch.Tensor): Incoming messages, shape [batch_size, n_nodes, n_nodes, d_message]    #changed
        ...
        coord_updates, node_updates = self.node_ff(coords, node_feats, node_mask, edge_feats)    #changed
        coords = coords + coord_updates
        node_feats = node_feats + node_updates

        messages = self.message_ff(coords, node_feats, node_mask, edge_feats, messages)    #changed
        edge_out = messages[:, :, :, : self.d_edge_out]    #changed
        messages_out = messages[:, :, :, self.d_edge_out :]    #changed

        edge_out = edge_feats + edge_out    #changed
        node_messages = messages_out[:, :, :, : self.n_attn_heads]
        coord_messages = messages_out[:, :, :, self.n_attn_heads : (self.n_attn_heads + self.d_coord_message)]

        node_feats = node_feats + self.node_attn(node_feats, node_messages, adj_matrix)    #TO DO
        coords = coords + self.coord_attn(coords, coord_messages, adj_matrix, node_mask)    #TO DO

        return coords, node_feats, edge_out, node_messages, coord_messages    #changed
...
class EquiInvDynamics(torch.nn.Module):
    def __init__(
        ...
        self._hparams = {
            ...
            "d_edge": d_edge,    #changed
            "n_layers": n_layers,
            "n_attn_heads": n_attn_heads,
            "d_message_hidden": d_message_hidden,
            "d_hidden": d_hidden,    #changed
        ...
        self.final_ff_block = NodeFeedForward(d_model, n_coord_sets, d_edge, coord_norm=coord_norm)    #changed   
        self.coord_norm = CoordNorm(n_coord_sets, norm=coord_norm)
        self.feat_norm = torch.nn.LayerNorm(d_model)
        self.bond_norm = torch.nn.LayerNorm(d_edge)    #changed
...
#TO DO
    def forward(self, coords, inv_feats, adj_matrix, edge_feats, atom_mask=None, cond_coords=None):    #changed
...
class SemlaGenerator(MolecularGenerator):
    ...
    def forward(
        ...
        pred_coords, pred_feats, pred_edges = self.dynamics(    #changed
            coords, atom_feats, adj_matrix, edge_feats, atom_mask=atom_mask, cond_coords=cond_coords    #changed
        )
